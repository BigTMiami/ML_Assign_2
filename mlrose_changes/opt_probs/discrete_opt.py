...
class DiscreteOpt(_OptProb):
    ...
    def __init__(self, length, fitness_fn, maximize=True, max_val=2, crossover=None, mutator=None):
        ...
        self.random_neighbor_indexes = np.arange(length)
        np.random.shuffle(self.random_neighbor_indexes)
        self.random_neighbor_indexes = self.random_neighbor_indexes.tolist()
    ...
    # AF Menninger 2/28/2022
    # Made adjustment for discrete RHC to randomly select each neighbor
    # only once.  Max_attempts should be set to lower than length of problem
    def set_random_next_neighbors(self):
        self.random_neighbor_indexes = np.arange(self.length)
        np.random.shuffle(self.random_neighbor_indexes)
        self.random_neighbor_indexes = self.random_neighbor_indexes.tolist()
    ...
    def random_next_neighbor(self):
        ...
        neighbor: array
            State vector of random neighbor.
        """
        if len(self.random_neighbor_indexes) == 0:
            return None
        neighbor = np.copy(self.state)

        i = self.random_neighbor_indexes.pop()
